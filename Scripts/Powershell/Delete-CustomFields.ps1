...
.DESCRIPTION
    ...
    Deletes specified custom fields. Removes all custom fields if no fields specified.
...
.EXAMPLE
    .\Delete-CustomFields.ps1 -VSAAddress 'https://vsa.example' -VSAUserName 'user1' -VSAUserPAT '01e0e010-1010-1010-b101-ca1beec10efc' -FieldsToRemove Field1, Field2
.NOTES
    Version 0.1
    Requires:
...
if ( Get-Module -ListAvailable -Name $ModuleName) {
    ...
    $ExistingFields = Get-VSACustomFields -VSAConnection $VSAConnection | Select-Object -ExpandProperty FieldName

    # If no FieldsToRemove specified remove all custom fields
    
    if ( 0 -eq $FieldsToRemove.Count)
    {
        $FieldsToRemove = $ExistingFields
    }
    $ExistingFields | Where-Object {$_ -in $FieldsToRemove} | ForEach-Object { Remove-VSACustomField -FieldName $_  -VSAConnection $VSAConnection -Confirm:$false}
