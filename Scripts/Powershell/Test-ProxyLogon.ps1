        ...
        process {
            foreach ($report in $InputObject) {
                ...
                if (-not ($report.Cve26855.Count -or $report.Cve26857.Count -or $report.Cve26858.Count -or $report.Cve27065.Count -or $report.Suspicious.Count)) {
                    ...
                    Write-Host ""
                    echo "Nothing suspicious detected" | Out-File "C:\Users\Gonz\Downloads\result.txt"
                    continue
                }

                if ($report.Cve26855.Count -gt 0) {
                    Write-Host "  [CVE-2021-26855] Suspicious activity found in Http Proxy log!" -ForegroundColor Red
                    echo "[CVE-2021-26855] Suspicious activity found in Http Proxy log. Review GetFile results" | Out-File "$WorkDir\result.txt"
                    if (-not $DisplayOnly) {
                ...
                if ($report.Cve26857.Count -gt 0) {
                    ...
                    echo "[CVE-2021-26857] Suspicious activity found in Eventlog. Review GetFile results" | Out-File "$WorkDir\result.txt"
                    if (-not $DisplayOnly) {
                ...
                if ($report.Cve26858.Count -gt 0) {
                    ...
                    echo "[CVE-2021-26858] Suspicious activity found in OAB generator logs. Review GetFile results" | Out-File "$WorkDir\result.txt"
                ...
                if ($report.Cve27065.Count -gt 0) {
                    ...
                    echo "[CVE-2021-27065] Suspicious activity found in ECP logs. Review GetFile results" | Out-File "$WorkDir\result.txt"
