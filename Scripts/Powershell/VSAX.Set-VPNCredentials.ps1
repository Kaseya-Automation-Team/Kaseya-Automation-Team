...
.PARAMETER AllUserConnection
    (Optional) Set the VPN connection for all users. If not selected, the VPN connection is created / removed for users with active sessions at the script execution time.
.EXAMPLE
...
if ( -not $Remove) {
    $splat.Add( 'ServerAddress', $ServerAddress)

    if ( -not [string]::IsNullOrEmpty($TunnelType) ) {
        $splat.Add( 'TunnelType', $TunnelType)
    }
    if ( -not [string]::IsNullOrEmpty($AuthenticationMethod) ) {
        $splat.Add( 'AuthenticationMethod', $AuthenticationMethod)
    }
    if ( -not [string]::IsNullOrEmpty($AuthenticationMethod) ) {
        $splat.Add( 'AuthenticationMethod', $AuthenticationMethod)
        if ( 'Eap' -ieq $AuthenticationMethod) {
            $L2tpPsk = $L2tpPsk.Trim()
            if ( -not [string]::IsNullOrEmpty($L2tpPsk) ) {
                $splat.Add( 'L2tpPsk', $L2tpPsk)
            }
        }
    }
    if ( -not [string]::IsNullOrEmpty($UseWinlogonCredential) ) {
        $splat.Add( 'UseWinlogonCredential', $UseWinlogonCredential)
    }
    if ( -not [string]::IsNullOrEmpty($RememberCredential) ) {
        $splat.Add( 'RememberCredential', $RememberCredential)
    }
}

Set-VPNCredentials @splat