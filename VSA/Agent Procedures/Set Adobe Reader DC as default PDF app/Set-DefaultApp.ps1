...
.DESCRIPTION
   Set File Type Association for all users.
.EXAMPLE
   .\Set-DefaultApp.ps1 -LogIt
.NOTES
   Version 0.1.1
...
function Register-FTA {
    ...
    $RegKey = 'HKCU:'
    if ( -Not [string]::IsNullOrEmpty($SID)) {$RegKey = "HKEY_USERS\$SID"}
...
Foreach ($Profile in $ProfileList)
{
    ...
    Set-FTA -ProgId AcroExch.Document.DC -Extension .pdf -SID $($Profile.SID)
    #####################################################################
