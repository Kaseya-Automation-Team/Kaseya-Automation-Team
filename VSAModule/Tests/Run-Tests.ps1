...
# $MyInvocation is NOT inherited by nested scriptblocks (e.g. a Where-Object filter gets its own,
# empty $MyInvocation for that anonymous invocation) -- capture the name into a plain variable
# first, since only plain variables close over the enclosing scope correctly.
$script:RunnerFileName = $MyInvocation.MyCommand.Name

# Build Invoke-Pester parameters
$PesterParams = @{
    # Exclude this runner's own file: '*Tests.ps1' also matches 'Run-Tests.ps1', which made
    # Pester invoke the runner itself as a test container (infinite self-recursion).
    Path = Get-ChildItem -Path $TestPath -Filter '*Tests.ps1' -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -ne $script:RunnerFileName } |
        Select-Object -ExpandProperty FullName
    Output = $Output
    # Always request PassThru from Pester so this script's own summary/exit-code logic below has a
    # result object to inspect; the caller's -PassThru switch (checked separately near the bottom)
    # controls only whether that object is also returned to the caller. Without this, a plain
    # `.\Run-Tests.ps1` with no switches got $null back and always reported "No test results
    # returned" / exit 1, even when every test passed.
    PassThru = $true
}
