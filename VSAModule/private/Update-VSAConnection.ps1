...
#>
    param (
        ...
        # Skip the "about to expire" check and renew unconditionally.
    ...
    if ($VSAConnection) {
        ...
        if (-not [string]::IsNullOrEmpty((Get-VSAPersistentToken))) {
            $SessionExpiration = $(Get-VSAPersistentSessionExpiration).AddMinutes(-1)
        } else {
    ...
    if ($Force -or ($SessionExpiration -le [datetime]::Now)) {
        ...
        if ($null -eq $VSAConnection) {
            ...
            $VSAUserPAT = Get-VSAPersistentPAT
            $VSAUserName = Get-VSAPersistentUserName
            $VSAServer = Get-VSAPersistentURI
        ...
        if ($null -eq $VSAConnection) {
            # The Connection is persistent
            Update-VSAPersistentField -Index 1 -Value $result.Token
            Update-VSAPersistentField -Index 4 -Value ($SessionExpiration.ToString('o', [System.Globalization.CultureInfo]::InvariantCulture))
        } else {
