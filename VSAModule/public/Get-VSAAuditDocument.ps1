function Get-VSAAuditDocument
{
    ...
    })  -f $AgentID, (Format-VSAPathSegment ($Path -replace '\\', '/'))
    ...
    if ($DownloadDocument) {
        ...
        # Download to a file: Invoke-VSARestMethod forwards -OutFile to the transport, which writes
        # the response body to disk. It has no -DownloadsFolder parameter, so passing that failed
        # every document download with "a parameter cannot be found" (F-27). Build the OutFile path
        # from the folder + the document's leaf name, mirroring Get-VSAStorageContent.
        $Params['OutFile'] = Join-Path -Path $DownloadsFolder -ChildPath (Split-Path -Path $Path -Leaf)
    } else {
