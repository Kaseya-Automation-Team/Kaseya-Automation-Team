function New-VSAOrganization {
    ...
    .DESCRIPTION
        Creates a new organization in the VSA environment.
    ...
    .PARAMETER DefaultDepartmentName
        Specifies Default Department Name. Default is 'root'.
    .PARAMETER DefaultMachineGroupName
        Specifies Default Machine Group Name. Default is 'root'.
    .PARAMETER OrgType
        Specifies Organization Type.
    .PARAMETER ParentOrgId
        Specifies Numeric Id of an existing organization that is set as the parent for the new one.
    .PARAMETER Website
    ...
    .PARAMETER ExtendedOutput
        Indicates whether to return extended output. If specified, returns the OrgId property of the newly created organization; otherwise, returns boolean result if the operation was successfull.
    .EXAMPLE
        New-VSAOrganization -VSAConnection $VSAConnection -OrgName 'My Organization' -OrgRef 'myorg'
    .INPUTS
        Accepts a piped Organization object.
    .OUTPUTS
        True if creation was successful; The OrgId property of the new Organization if the ExtendedOutput switch is specified.
    .NOTES
        Version 0.1.1
#>
    ...
    param (
        ...
        [parameter(Mandatory=$true, ValueFromPipelineByPropertyName=$true, HelpMessage = "Specify the name of the organization.")]
        [ValidateNotNullOrEmpty()]
        [string] $OrgName,

        [parameter(Mandatory=$true, ValueFromPipelineByPropertyName=$true, HelpMessage = "Unique string to reference the organization. Usually, shorten name or acronym.")]
        [ValidateNotNullOrEmpty()]
        [string] $OrgRef,
    ...
    if ( 0 -lt $CustomFieldArray.Count )  {
    ...
    # Define parameters for Invoke-VSARestMethod
    [hashtable]$Params = @{
        VSAConnection  = $VSAConnection
        URISuffix      = $URISuffix
        Method         = 'POST'
...
Export-ModuleMember -Function New-VSAOrganization